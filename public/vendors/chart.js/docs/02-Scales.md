...
display | Boolean | true | If true, show the scale including gridlines, ticks, and labels. Overrides *gridLines.display*, *sc bel.display*, and *ticks.display*.
...
**sc bel** | Object | | See [scale title configuration](#scales-scale-title-configuration) section.
...
The grid line configuration is nested under the scale configuration in the `sc bel` key. It defines options for the scale title.
