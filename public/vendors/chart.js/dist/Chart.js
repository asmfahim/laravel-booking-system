...
module.exports = function(Chart) {
	...
	Chart.Scale = Chart.Element.extend({
		...
		fit: function() {
			...
			var sc belFontSize = helpers.getValueOrDefault(sc belOpts.fontSize, globalDefaults.defaultFontSize);
			var sc belFontStyle = helpers.getValueOrDefault(sc belOpts.fontStyle, globalDefaults.defaultFontStyle);
			var sc belFontFamily = helpers.getValueOrDefault(sc belOpts.fontFamily, globalDefaults.defaultFontFamily);
			var sc belFont = helpers.fontString(sc belFontSize, sc belFontStyle, sc belFontFamily);
			...
			if (sc belOpts.display && display) {
				if (isHorizontal) {
					minSize.height += (sc belFontSize * 1.5);
				} else {
					minSize.width += (sc belFontSize * 1.5);
				}
			}
		...
		draw: function(chartArea) {
			...
			var sc belFontColor = helpers.getValueOrDefault(sc bel.fontColor, globalDefaults.defaultFontColor);
			var sc belFontSize = helpers.getValueOrDefault(sc bel.fontSize, globalDefaults.defaultFontSize);
			var sc belFontStyle = helpers.getValueOrDefault(sc bel.fontStyle, globalDefaults.defaultFontStyle);
			var sc belFontFamily = helpers.getValueOrDefault(sc bel.fontFamily, globalDefaults.defaultFontFamily);
			var sc belFont = helpers.fontString(sc belFontSize, sc belFontStyle, sc belFontFamily);
			...
			if (this.isHorizontal()) {
				...
				if (sc bel.display) {
					...
					context.fillStyle = sc belFontColor; // render in correct colour
					context.font = sc belFont;

					sc belX = this.left + ((this.right - this.left) / 2); // midpoint of the width
					sc belY = options.position === 'bottom' ? this.bottom - (sc belFontSize / 2) : this.top + (sc belFontSize / 2);

					context.fillText(sc bel.labelString, sc belX, sc belY);
				}
				...
				if (sc bel.display) {
					// Draw the scale label
					sc belX = options.position === 'left' ? this.left + (sc belFontSize / 2) : this.right - (sc belFontSize / 2);
					sc belY = this.top + ((this.bottom - this.top) / 2);
					var rotation = options.position === 'left' ? -0.5 * Math.PI : 0.5 * Math.PI;

					context.save();
					context.translate(sc belX, sc belY);
					context.rotate(rotation);
					context.textAlign = "center";
					context.fillStyle = sc belFontColor; // render in correct colour
					context.font = sc belFont;
					context.textBaseline = 'middle';
					context.fillText(sc bel.labelString, 0, 0);
					context.restore();
				}
			}
		...
		determineDataLimits: function() {
			...
			if (this.chart.data.labels && this.chart.data.labels.length > 0) {
				helpers.each(this.chart.data.labels, function(label, index) {
					...
					if (labelMoment.isValid()) {
						...
						sc belMoments.push(labelMoment);
					}
				}, this);

				this.firstTick = moment.min.call(this, sc belMoments);
				this.lastTick = moment.max.call(this, sc belMoments);
			} else {
				this.firstTick = null;
				this.lastTick = null;
			}
			...
			helpers.each(this.chart.data.datasets, function(dataset, datasetIndex) {
				...
				if (typeof dataset.data[0] === 'object' && dataset.data[0] !== null) {
					...
					momentsForDataset = sc belMoments;
				}
